...
def add_parser(subparsers: argparse._SubParsersAction) -> None:
    ...
    parser.add_argument(
        "--template",
        default=None,
        help="Name of the template repository to clone (defaults to the .mctrc default_template)",
    )
...
def run(args: argparse.Namespace) -> None:
    ...
    template = args.template or data.get("default_template")
    if not template:
        print(
            "No --template given and no default_template set in .mctrc. "
            "Run `mct set default <template_name>` or pass --template.",
            file=sys.stderr,
        )
        raise SystemExit(1)

    base_url = template_site_path.rstrip("/") + "/" + username
    template_url = f"{base_url}/{template}.git"
    ...
    print(f"Checking that template {template} exists...")
